...
/**
 * 确认收货并发测试：两个请求同时确认同一 PENDING_RECEIVE 子订单，
 * 条件更新抢占保证仅一个成功，数据库终态为 FINISHED。
 */
@SpringBootTest
@ActiveProfiles("test")
@Sql(scripts = "/sql/schema.sql", executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
@Sql(scripts = "/sql/cleanup.sql", executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
class ReceiveConcurrencyTest {
    ...
    @Test
    void concurrentReceiveOnlyOneSucceeds() throws Exception {
        ...
        shipDTO.setLogisticsNo("SF111");
        fulfillmentService.ship(farmerId, subOrder.getSubOrderNo(), shipDTO);
        assertThat(subOrderMapper.selectById(subOrder.getId()).getStatus()).isEqualTo(2);

        CountDownLatch ready = new CountDownLatch(2);
        CountDownLatch start = new CountDownLatch(1);
        ExecutorService executor = Executors.newFixedThreadPool(2);
        try {
            Future<Object> f1 = executor.submit(() -> receiveWhenSignaled(buyerId, subOrder.getSubOrderNo(), ready, start));
            Future<Object> f2 = executor.submit(() -> receiveWhenSignaled(buyerId, subOrder.getSubOrderNo(), ready, start));
            ready.await();
            start.countDown();
    ...
    private Object receiveWhenSignaled(long buyerId, String subOrderNo,
                                       CountDownLatch ready, CountDownLatch start) {
        ready.countDown();
        try {
            start.await();
            return fulfillmentService.receive(buyerId, subOrderNo);
